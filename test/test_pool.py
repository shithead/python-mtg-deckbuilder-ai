...
def test_delete_last_card():
    ...
    pool.insert(card1)
    del pool.current
    assert pool.current is None and pool.is_first and pool.is_last and pool.size == 0
...
def test_delete_cards():
    ...
    del pool.current
    assert pool.size == 2
    assert pool.first is card2
    assert pool.first.next is card3
    assert pool.first.prev is card3
    assert pool.last is card3
    assert pool.last.next is card2
    assert pool.last.prev is card2
    pool.last
    del pool.current
    assert pool.first is card2
    assert pool.first.next is card2
    assert pool.first.prev is card2
